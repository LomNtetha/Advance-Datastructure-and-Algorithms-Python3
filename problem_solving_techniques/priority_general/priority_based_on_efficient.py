...
"""
21. Dijkstra's Algorithm (Shortest Path in a Weighted Graph)
Problem Statement:
Given a graph with nodes and weighted edges, find the shortest path from a source node to all other nodes.
...
{'A': 0, 'B': 1, 'C': 3, 'D': 4}
Explanation: The shortest paths from node 'A' to all other nodes.

Technique: Dijkstra's Algorithm (Graph Traversal using Priority Queue)
Time Complexity: 
O(ElogV), where 
...
class Solution:
    def dijkstra(self, graph, source):
        """
        Find the shortest path from the source to all other nodes using Dijkstra's algorithm.
...
print(Solution().dijkstra(graph, 'A'))  # Output: {'A': 0, 'B': 1, 'C': 3, 'D': 4}
